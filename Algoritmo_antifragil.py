...
class Calculo(threading.Thread):
    ...
    def __init__(self, An, meses, kind, iter, mutex, capMax, pvpmin, pvpmax, liquidez, fixos, salvo):
        self.An = An
        self.meses = meses
        self.kind = kind
        self.iter = iter
    ...
    def selectAtivos(self):
        ...
        if self.pvpmin != 0 and self.pvpmax != 0 and self.liquidez >= 0:
            df_filtered = df[(df_transpose.iloc[5, 0:] <= self.pvpmax) & (df_transpose.iloc[5, 0:] >= self.pvpmin) &
                             (df_transpose.iloc[4, 0:] >= self.liquidez) & (df_transpose.iloc[6, 0:] >= 0) &
                            (df_transpose.iloc[7, 0:] > 0) & (df_transpose.iloc[8, 0:] > 0) & (df_transpose.iloc[7, 0:] >= df_transpose.iloc[8, 0:])]
            df_filtered_transpose = df_filtered.T
            self.num_column = df_filtered_transpose.shape[1]
            print(df_filtered_transpose)
        else:
            # FILTRANDO SOMENTE PELA ANTIFRAGILIDADE
            df_filtered = df[(df_transpose.iloc[6, 0:] >= 0) & (df_transpose.iloc[7, 0:] > 0) & (df_transpose.iloc[8, 0:] > 0) & (df_transpose.iloc[7, 0:] >= df_transpose.iloc[8, 0:])]
            df_transpose = df_filtered.T
            self.num_column = df_transpose.shape[1]
            #print(df_transpose)
        ...
        if self.pvpmin != 0 and self.pvpmax != 0 and self.liquidez >= 0:
            ordenado = df_filtered_transpose.iloc[4:, 0:].mean().sort_values(ascending=False)
            #ordenado = df[(df_filtered_transpose.iloc[2, 0:] >= 0) ].sort_values(ascending=False)
            # OBTENDO A MÉDIA DAS FUNÇÕES
            self.medias = df_filtered_transpose.iloc[4:, 0:].mean()
        else:
            ordenado = df_transpose.iloc[4:, 0:].mean().sort_values(ascending=False)
            #ordenado = df[(df_transpose.iloc[2, 0:] >= 0)].sort_values(ascending=False)
            # OBTENDO A MÉDIA DAS FUNÇÕES
            self.medias = df_transpose.iloc[4:, 0:].mean()
        '''
        # FAZENDO A ORDENAÇÃO DAS ANTIFRÁGEIS
        if self.pvpmin != 0 and self.pvpmax != 0 and self.liquidez >= 0:
            ordenado_AF = df_filtered_transpose.iloc[6, 0:].sort_values(ascending=True)

        else:
            ordenado_AF = df_transpose.iloc[6, 0:].sort_values(ascending=True)

        #print(ordenado_AF.shape[0]) # Verifica a qnt de fundos filtrados
        # OBTENDO O EIXO X --> INTERVALO DE t(TEMPO)
        intervalo = [x for x in range(1, (df_transpose.shape[0])-8)] # 8 é a subtração dos termos que não fazem parte da contagem
        # CONVERTENDO PARA MATRIZ
        self.inter = array(intervalo)
        #print(inter)

        '''
        # OBTENDO AS FUNÇÕES(HISTÓRICO DE VARIAÇÃO)
        if self.pvpmin != 0 and self.pvpmax != 0 and self.liquidez != 0:
            # ARMAZENANDO ARRAYS DENTRO DA LISTA
            #matriz_dados = mat(df_filtered_transpose.iloc[3:, 0:].T.values).astype(float)
            self.matriz_dados = [array(x) for x in (df_filtered_transpose.iloc[4:, 0:].T.values).astype(float)]

        else:
            #matriz_dados = mat(df_transpose.iloc[3:, 0:].T.values).astype(float)
            self.matriz_dados = [array(x) for x in (df_transpose.iloc[4:, 0:].T.values).astype(float)]
        '''
        #print(df_filtered_transpose.iloc[7:, 0].shape[0])
        #print(len(matriz_dados[0]))
        #print(len(intervalo))
        ...
        #self.best_mean = array(ordenado[0:self.An])
        #self.best_mean_index = list(ordenado[0:self.An].index)
        self.best_name = []
        self.best_AF_value = [] #armazena os valores antifrágeis
        self.best_AF = [] #armazena as variações
        self.best_AF_index = list(ordenado_AF[0:self.An].index) #Coleta os índices
        self.best_mean = []
        #print(self.best_AF_index)
        self.best_tipoDoPapel = []
        self.dividendo = []
        self.cota = []
        self.liquidez = []
        self.pvp = []
        # Média das funções, transformando em vetor
        #self.mean = array(self.medias)
        #print('-' * 20, 'Média das funções', '-' * 20)
        #print(self.mean)

        # Obtendo as funções
        # POLINOMIAIS
        #p1 = polyfit(inter, matriz_dados[0], 100)
        #print(p1)
        #plt.plot(inter, matriz_dados[0], 'o')
        #plt.plot(inter, polyval(p1, inter), '-')

        # RBF
        #py.figure(1)
        #py.clf()
        #py.plot(self.inter, self.matriz_dados[0], 'o')
        # GERANDO UM NÚMERO DE VALORES IGUALMENTE DISTRIBUÍDOS NO INTERVALO ESPECIFICADO
        #xnew = linspace(1, self.inter.shape[0], 3000)
        #print(self.inter.shape[0], len(self.matriz_dados[0]))
        if self.num_column >= self.An:
            for i in range(0, self.An):

                # INTERPOLAÇÃO RBF
                #radFunc = interpolate.Rbf(self.inter, self.matriz_dados[i], function='gaussian')
                # FUNÇÃO DESCOBERTA
                #function = radFunc(xnew)
                # PLOTANDO AS FUNÇÕES
                #py.plot(xnew, function, label='gaussian')
                #py.plot(self.inter, self.matriz_dados[i], label='true')
                #py.legend(loc='lower right')
                #py.show()
                # TESTE
                #print(radFunc(2), self.matriz_dados[0][1])
                # FUNÇÃO DA MÉDIA
                #funcMean = radFunc(self.inter.mean())
                if self.pvpmin != 0 and self.pvpmax != 0 and self.liquidez != 0:
                    self.best_name.append(df_filtered_transpose.iloc[0, i])
                    self.best_tipoDoPapel.append(df_filtered_transpose.iloc[1, i])
                    self.dividendo.append(df_filtered_transpose.iloc[2, i])
                    self.cota.append(df_filtered_transpose.iloc[3, i])
                    self.liquidez.append(df_filtered_transpose.iloc[4, i])
                    self.pvp.append(df_filtered_transpose.iloc[5, i])
                    self.best_AF_value.append(df_filtered_transpose.iloc[6, i])
                    self.best_AF.append(df_filtered_transpose.iloc[9:, i])
                    self.best_mean.append(df_filtered_transpose.iloc[9:, i].mean())
                else:
                    self.best_name.append(df_transpose.iloc[0, i])
                    self.best_tipoDoPapel.append(df_transpose.iloc[1, i])
                    self.dividendo.append(df_transpose.iloc[2, i])
                    self.cota.append(df_transpose.iloc[3, i])
                    self.liquidez.append(df_transpose.iloc[4, i])
                    self.pvp.append(df_transpose.iloc[5, i])
                    self.best_AF_value.append(df_transpose.iloc[6, i])
                    self.best_AF.append(df_transpose.iloc[9:, i])
                    self.best_mean.append(df_transpose.iloc[9:, i].mean())
        else:
        ...
        #for i in self.best_mean_index:
            #lista_best.append(df_transpose.iloc[6:, i])
        ...
        self.matriz_best = mat(self.best_AF).astype(float)
        ...
        return self.best_AF, self.best_AF_index, self.best_mean#, self.best_mean_index

    def func(self, x):
        return exp(-(1/2*7.0**2)*(sqrt((x-124)**2)))#x*exp(-1.5*x**2)
    ...
    def selectAtivosAcertivo(self):
        ...
        # Gerando uma matriz com os dados dos melhores papéis
        self.matriz_best = mat(lista_best).astype(float)
        #print('-'*20, 'Matriz dos melhores papéis', '-'*20)
        #print(self.matriz_best)
        #print(self.matriz_best.shape)
        #print('-'*20, 'Melhores médias', '-'*20)
    ...
    def run(self): #Antigo solve

        # Gerando a Matriz de Covariância
        matrix_Cov = cov(self.matriz_best, ddof=0)
        #print('-'*20, 'Matriz da Covariância', '-'*20)
        #print(matrix_Cov)

        # EQUAÇÃO DO RETORNO
        # Transformando a lista das melhores médias em matriz
        #matrix_best_mean = mat(self.best_mean).astype(float)
        # Transformando a lista de dividendos em matriz
        #self.matriz_dividendo = mat(self.dividendo).astype(float)
        ...
        #print(self.matriz_dividendo)
        #print(matriz)
        matriz = mat(self.best_mean).astype(float)
        ...
        with open(f'E:/OneDrive/Cursos Python/MONETA/Resultados/Dados_Resultados_AF_{self.meses}_meses_{self.kind}_{self.An}papeis.csv', 'w', newline='') as dados:
            ...
            for i in range(0, self.iter):
                with self.mutex:
                    ...
                    risco = Ain.dot(matrix_Cov).dot(Ain.T) #Resulta na somatória das variâncias com as devidas porcentagens
                    #print('-' * 20, 'Risco', '-' * 20)
                    #print(risco)
    ...
    def ganhoReal(self):
        ...
        if capMax != 0:
            desvio = array([retorno*100, df.iloc[self.linha, self.An]])
        else:
            desvio = array([retorno * 100, df.iloc[0, self.An]])
    ...
    def quantAtivos(self):
        ...
        if self.capMax == 0:
            ...
            with self.mutex:
                ...
                matriz_var = array(name_var)
                matriz_cota = array(self.cota)
                ...
                vec_valor = array(self.valores)
                vec_dividendo = array(self.dividendo)
            ...
            while True:
                with self.mutex:
                    ...
                    if self.total_invest <= capMax:
                        ...
                        # SHELVE PARA GUARDAR OS DADOS
                        db_shelve = shelve.open('Invest_AF.db')
                        db_shelve['nomes'] = self.best_name
                        db_shelve['qnt_cotas'] = self.valores
                        db_shelve['valorPapel'] = self.cota
                        db_shelve['investimento_min'] = self.investimento_min
    ...
    def comparaInvest(self):
        ...
        db = shelve.open('ComparaInvest_AF.db')
