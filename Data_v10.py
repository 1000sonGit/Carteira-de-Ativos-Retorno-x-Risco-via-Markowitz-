    ...
    def dadosCotacoes(self):#antigo run
        ...
        with tqdm(total=len(self.list_fundos)) as self.pbar:
            ...
            for fundo in self.list_fundos:
                ...
                #with self.mutex:
                if passa != 1:
                    # Obtendo somente a última cotação(preço de fechamento do dia anterior)
                    self.cota.append(data_filtro19.iloc[-1, 13])
                    self.lastVolume.append(data_filtro19.iloc[-1, 18])

                    # Obtendo todas as variações do período, volume do período e preço do papel
                    ...
                    for i in range(1, len(data_filtro19.iloc[0:, 0])+1):
                        ...
                        self.preco.append(data_filtro19.iloc[-i, 13])
                        self.volume.append(data_filtro19.iloc[-i, 18])
                        # ARMAZENANDO AS VARIAÇÕES ATÉ A QUANTIDADE DA MÉDIA LONGA
                        if i <= self.long+1:
                    ...
                    if len(self.variacao) < self.dias:
                        for i in range(1, len(data_filtro18.iloc[0:, 0])+1):
                            ...
                            self.preco.append(data_filtro18.iloc[-i, 13])
                            self.volume.append(data_filtro18.iloc[-i, 18])
                            if len(self.variacao) == self.dias:
                                break
                    with self.mutex:
                        ...
                        if len(self.variacao) == self.dias:
                            ...
                            vol = array([float(y) for y in self.volume])
                            vol_medio = vol.mean()
                            self.unilist.append(vol_medio)
                            self.volume.clear()

                            # Adicionando o último volume
                            # Condição para identificar volume negativo
                            if self.variacao[0] > 0:
                                self.unilist.append(self.lastVolume[0])
                            else:
                                self.unilist.append(self.lastVolume[0] * -1)
                            self.lastVolume.clear()

                            # Adicionando o p/vp, importante para ver se vale a pena comprar o papel
                            self.unilist.append(float(pvp[6].replace(',', '.')))

                            # CONVERTENDO A PREÇO(f(x)) PARA ARRAY
                            varia = [float(x) for x in self.preco]
                            var = array(varia)
                            # CALCULANDO A MÉDIA DA FUNÇÃO (PREÇO MÉDIO)
                            media_func = var.mean()
                            self.unilist.append(media_func)
                            self.preco.clear()
                            ...
                            # SE A ANÁLISE FOR SEMANAL, CALCULAR A VARIAÇÃO SEMANAL DO FUNDO
                            elif self.kind == 'semanal':
                                # DIVIDINDO OS DADOS EM SEMANAS
                                lista_qnts = []
                                n, now = 0, 0
                                for c, s in enumerate(self.lista_dias.week):
                                    if c == 0:
                                        now = s
                                        n += 1
                                    else:
                                        if now == s and c != len(self.lista_dias) - 1:
                                            n += 1
                                        elif now != s and c != len(self.lista_dias) - 1:
                                            now = s
                                            lista_qnts.append(n)
                                            n = 1
                                        else:
                                            n += 1
                                            lista_qnts.append(n)
                                # Invertendo a lista, deixando a data recente no começo da lista (Deixar de lado)
                                #list_normal = lista_qnts[::-1]
                                list_normal = lista_qnts[::]

                                # Algoritmo para somar as variações deixando-as semanais
                                i, soma, c = 0, 0, 1
                            ...
                            # SE A ANÁLISE FOR MENSAL, CALCULAR A VARIAÇÃO MENSAL DO FUNDO
                                ...
                                lista_qnts = []
                                n, now = 0, 0
                                # self.lista_dias.month: mostrará os meses dos dias úteis. Ex: 20-5-2019| 21-5-2019 mostra [5, 5...]
...
class Actions(QWidget):
    ...
    def __init__(self, parent=None):#, mutex):
    ...
    def onButtonClick(self):
        ...
        objDados = WebScraping(meses, kind, descartados, path_old, path_now)#, self.mutex)
        objDados.valueChanged.connect(self.onCountChanged)
        objDados.time()
        objDados.listaFundos()
        objDados.start()
        #threads.append(objDados)
        #for thread in threads:
        #    thread.join()
        objDados.dadosSelect()
